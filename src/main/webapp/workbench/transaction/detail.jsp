<%@ page import="com.lz.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.lz.crm.workbench.domain.Tran" %>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%
String basePath = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort() + request.getContextPath() + "/";
	List<DicValue> dicValueList=(List<DicValue>)application.getAttribute("stage");
	Map<String,String> stringStringMap=(Map<String,String>)application.getAttribute("stringMap");
int boundary=0;
	for (int i=0;i<dicValueList.size();i++)
{
	if ("0".equals((String)stringStringMap.get(dicValueList.get(i).getValue())))
	{
		 boundary=i;
		break;
	}
...
function changeStage(stage,i) {
	$.ajax({
		url:"workbench/clue/changeStage.do",
		data:{
			"stage":stage,
			"id":"${tran.id}",
			"money" : "${tran.money}",
		...
		success:function (data){
			...
			$("#editBy").html(data.edit1By)
			ListTransactionHistory(1,2)
		changeIcon(i)
		}
	})
}
function changeIcon(index) {
                ...
                if(index>=<%=boundary%>){
                    ...
                    for(var i=0;i<<%=dicValueList.size()%>;i++){
	...
	function ListTransactionHistory(pageNum,pageSize) {

			$.ajax({
				url:"workbench/clue/ListTransactionHistory.do",
				data:{
					"pageNum":pageNum,
					"pageSize":pageSize,
					"tranId":"${tran.id}"
				...
				success:function (data){
					...
					$.each(data.activityList,function (i, activity) {
						...
						html+='    <td>'+activity.money+'</td>';
						html+='    <td>'+activity.possibility+'</td>';
						html+='    <td>'+activity.expectedDate+'</td>';

						html+='    <td>'+activity.createTime+'</td>';
						html+='    <td>'+activity.createBy+'</td>';
					...
					$("#historyPage").bs_pagination({
						...
						onChangePage : function(event, data){
							ListTransactionHistory(data.currentPage , data.rowsPerPage);
						}
	...
	//展现detail
			//当前阶段$("#stage").prop("class","glyphicon glyphicon-map-marker mystage")
			//前：绿色 $("[name=stage]:").prop("class","glyphicon glyphicon-ok-circle mystage")
//后：灰色
		/* function：当前 前 后 请求 刷新detail 数据与阶段对应，怎么绑定 改交易 加历史 刷新历史
		页面加载:阶段数据绑定在id上 函数绿灰
		index带参数 阶段数据
		})*/
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		<%
			...
			String stageCurrent=tran.getStage();
			for (int i=0;i<dicValueList.size();i++)
			{
				if (stageCurrent.equals(dicValueList.get(i).getValue()))
				{point=i;
				break;}
			}
			if("0".equals(stringStringMap.get(stageCurrent))){
				...
				for(int i=0;i<dicValueList.size();i++){
					...
					String listPossibility = stringStringMap.get(listStage);
					...
					if("0".equals(listPossibility)){
						...
						//如果是当前阶段
						if(listStage.equals(stageCurrent)){
...
%>	<span id="<%=i%>" onclick="changeStage('<%=listStage%>','<%=i%>')" class="glyphicon glyphicon-ok-circle mystage"
			data-toggle="popover" data-placement="bottom"
			data-content="<%=listStage%>" style="color: #90F790;">
				</span>
		-----------
		<%
			//如果大于当前阶段
...
-->
		...
		<!--ajax请求，，，要一个stage集合，，，，
		遍历集合 如果等于当前 记下下标current
		如果possibility等于0 记下下标poss0
if（当前equals“0”）
{
for（全部）
{
...
{尖图案}
		...
		-->
		<!--{ }数据值变，数量变用数组，集合，链表关键能找到，制造出循环初坐标和末坐标，
		关键也就是分类，所以找特点分类，找不出就制造一一对应来分类。-->

...
<script type="text/javascript" src="jquery/bootstrap_3.3.0/js/bootstrap.min.js"></script>
	...
	$(function(){
		pageClueRemarkList(1,2);
		$("#saveRemark").click(function () {

			saveClueRemark();
		});
		showClueActivity(1,2);
		$("#addActivityClue").click(function () {
			addActivityClue();
		})
		$("#checkboxAll").click(function () {
			$("[name=checkboxOne]").prop("checked",this.checked)
		})
		$("#listActivity").on("click","[name=checkboxOne]",function () {
			$("#checkboxAll").prop("checked",$("[name=checkboxOne]:checked").length==$("[name=checkboxOne]").length)
		})
		$("#searchName").keydown(function (event) {
			...
			if (event.keyCode==13)
			{
				...
				activityPageListRelate(1,2)
				return false;
		}
		})
		$("#relateActivity").click(function () {
			activityPageListRelate(1,2);
		})
	...
	function addActivityClue() {
		var activityId=[];
		for (i=0;i<$("[name=checkboxOne]:checked").length;i++)
activityId.push($($("[name=checkboxOne]:checked")[i]).val());
		$.ajax(
				{
					url: "workbench/clue/saveClueActivity.do",
					data:{"clueId":"<%=id%>",
	...
	function showClueActivity( pageNum, pageSize ) {
				...
				$.ajax({
					...
					success:function (data){
						...
						html+='	<td><a onclick="deleteRelate(\''+activity.id+'\')" href="javascript:void(0);"';
						html+='style="text-decoration: none;">';
							//html += '<td><a href="javascript:void(0);" onclick="unbund(\''+n.id+'\')"
							// style="text-decoration: none;"><span class="glyphicon glyphicon-remove">
							// </span>解除关联</a></td>';

							html+='<span class="glyphicon glyphicon-remove"></span>解除关联</a></td>';
						html+='	</tr>';
						})
						$("#showClueActivity").html(html);
						var totalPages;
						totalPages=data.total%pageSize==0?data.total/pageSize:parseInt(data.total/pageSize)+1;

						$("#pageClueActivity").bs_pagination({
							...
							onChangePage : function(event, data){
								showClueActivity(data.currentPage , data.rowsPerPage);
							}
	...
	function activityPageListRelate( pageNum, pageSize ) {
		$("#checkboxAll").prop("checked",false);
		$.trim($("#searchName").val($("#hiddenName").val())),
				$.ajax({
					url:"workbench/clue/activityPageListRelate.do",
					data:{
						"pageNum":pageNum,
						"pageSize":pageSize,
...
//<td><a style="text-decoration: none; cursor: pointer;" onclick="window.location.href='detail.html';">发传单</a></td>
						...
						$("#pageActivity").bs_pagination({
							...
							showRowsDefaultInfo: true,
							onChangePage : function(event, data){
								activityPageListRelate(data.currentPage , data.rowsPerPage);
							}
	...
	function deleteRelate(activityId) {
		$.ajax(
				{
					url: "workbench/clue/deleteRelate.do",
	...
	function pageClueRemarkList( pageNum, pageSize ) {

		$.ajax({
			url:"workbench/clue/pageClueRemarkList.do",
			data:{
				"pageNum":pageNum,
				"pageSize":pageSize,
				"clueId":"<%=id%>"
...
<input id="hiddenName" type="hidden">
		...
		<div class="modal-dialog" role="document" style="width: 80%;">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button id="addActivityClue" type="button" class="btn btn-primary">关联</button>
				</div>
			</div>
		</div>
	</div>
    ...
    <div class="modal fade" id="editClueModal" role="dialog">
        <div class="modal-dialog" role="document" style="width: 90%;">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    < m class="form-horizontal" role="form">
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="window.location.href='workbench/clue/getClueOne.do?cid=<%=id%>';"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
	...
	<div>
		<div style="position: relative; top: 60px; left: 40px;">
			...
			<div style="position: relative;top: 0px;">
				<table id="activityList" class="table table-hover" style="width: 900px;">
					<thead>
						<tr style="color: #B3B3B3;">
							<td>名称</td>
							<td>开始日期</td>
				...
				<div id="pageClueActivity"></div>
			</div>
			<div id="activityPage"></div>
			<div>
				<a id="relateActivity" href="javascript:void(0);" data-toggle="modal" data-target="#bundModal" style="text-decoration: none;"><span class="glyphicon glyphicon-plus"></span>关联市场活动</a>
			</div>
		</div>
	</div>
